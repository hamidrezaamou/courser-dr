...
@section('content')
    ...
    <div class="meta">
        <div><span class="label">نام بیمار:</span><span class="value">{{ fa_digits($surgery->patient_name) }}</span></div>
        <div><span class="label">کد ملی:</span><span class="value">{{ fa_digits($surgery->national_code) }}</span></div>
        <div><span class="label">موبایل:</span><span class="value" dir="ltr">{{ fa_digits($surgery->mobile) }}</span></div>
        <div><span class="label">تاریخ عمل:</span><span class="value">{{ fa_digits($surgeryJalali) }}</span></div>
        <div style="grid-column: span 2;"><span class="label">نوع عمل:</span><span class="value">{{ fa_digits($surgery->surgery_type ?: '—') }}@if($surgery->eye_side) · {{ fa_digits($surgery->eye_side) }}@endif</span></div>
    </div>
    <div class="rx-box">
        <strong>Rx</strong>
        @if($latestPrescription && $latestPrescription->items->isNotEmpty())
            <table class="rx-items">
                ...
                <tbody>
                    @foreach($latestPrescription->items as $item)
                        <tr>
                            <td><strong>{{ fa_digits($item->drug_name) }}</strong>@if($item->usage_type)<br><small>{{ $item->usage_type }}</small>@endif</td>
                            <td>{{ fa_digits(trim(($item->dosage ?: '').' '.($item->frequency ?: ''))) ?: '—' }}</td>
                            <td>{{ fa_digits(trim(($item->meal_timing ? $item->mealTimingLabel() : '').($item->duration ? ' · '.$item->duration : ''))) ?: '—' }}</td>
                            <td>{{ fa_digits($item->instructions ?: '—') }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
            @if($latestPrescription->notes)
                <p class="rx-hint"><strong>یادداشت:</strong> {{ fa_digits($latestPrescription->notes) }}</p>
            @endif
        @else
    ...
    <div class="rx-foot">
        <div class="rx-qr">
            @if(!empty($qrDataUri))
                <img src="{{ $qrDataUri }}" alt="کد QR نسخه">
            @endif
            <span dir="ltr">RX-{{ $surgery->id }}</span>
        </div>
        <div class="rx-sign">
            @if(!empty($signatureUrl))
                <img src="{{ $signatureUrl }}" class="signature" alt="امضای پزشک">
            @endif
            @if(!empty($stampUrl))
                <img src="{{ $stampUrl }}" class="stamp" alt="مهر مطب">
            @endif
            <div class="line"></div>
