    ...
    $buildPayload = function ($patient) {
        ...
        return [
            'json' => json_encode([
                ...
                'mobileSecondary' => $patient->mobile_secondary ?: null,
...
@else
        ...
        <div class="patient-grid grid grid-cols-1 gap-3 p-3 md:grid-cols-2 md:gap-4 lg:grid-cols-3 xl:grid-cols-4">
            @foreach ($patients as $patient)
                ...
                <article class="patient-card">
                    <span class="patient-card__corner">{{ $initial }}</span>
                    <div class="patient-card__top">
                        <div class="patient-card__info">
                            <h4 class="patient-card__name">{{ $patient->name }}</h4>
                            <p class="patient-card__code ltr-data" dir="ltr">{{ $patient->national_code }}</p>
                            <p class="patient-card__phone ltr-data" dir="ltr">{{ $patient->mobile }}</p>
                        </div>
                    ...
                    <div class="patient-card__foot">
                        <div class="patient-card__actions">
                            <button type="button" class="patient-card__act patient-card__act--tool row-toolbox-trigger" title="ابزار" aria-label="ابزار" data-toolbox-trigger data-toolbox-b64="{{ base64_encode($payload['json']) }}">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-4.35-4.35M11 18a7 7 0 100-14 7 7 0 000 14z" />
                                </svg>
                            </button>
                            <a href="{{ route('appointments.create', $patient) }}" class="patient-card__act patient-card__act--edit" title="ثبت ویزیت" aria-label="ثبت ویزیت">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                            ...
                            <a href="{{ route('patients.show', $patient) }}" class="patient-card__act patient-card__act--file" title="پرونده بیمار" aria-label="پرونده بیمار">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
                                </svg>
                            </a>
                        </div>
                        <span class="patient-card__surgery-badge">تعداد اعمال: {{ number_format($surgeryTotal) }}</span>
                    </div>
                </article>
            @endforeach
        </div>
    ...
    <div class="patient-view patient-view--list" hidden>
        <div class="patient-list flex flex-col">
            @foreach ($patients as $patient)
                @php $payload = $buildPayload($patient); @endphp
                <div
                    role="button"
                    tabindex="0"
                    ...
                    data-search="{{ strtolower($patient->name.' '.$patient->national_code.' '.$patient->mobile) }}"
                    data-toolbox-b64="{{ base64_encode($payload['json']) }}"
                >
                    <div class="patient-row__avatar flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-sm font-extrabold text-white shadow-sm">{{ mb_substr($patient->name, 0, 1) }}</div>
                    <div class="patient-row__body min-w-0 flex-1">
                        <div class="patient-row__name truncate text-sm font-bold" style="color: var(--ink);">{{ $patient->name }}</div>
                    ...
                    <div class="patient-row__badges hidden shrink-0 flex-wrap gap-1 sm:flex">
                        @if ($patient->upcoming_visits_count > 0)
                            <span class="patient-badge patient-badge--visit rounded-full px-2 py-0.5 text-[10px] font-extrabold">{{ $patient->upcoming_visits_count }} ویزیت</span>
                        @endif
                        @if ($patient->upcoming_surgeries_count > 0)
                            <span class="patient-badge patient-badge--surgery rounded-full px-2 py-0.5 text-[10px] font-extrabold">{{ $patient->upcoming_surgeries_count }} عمل</span>
                        @endif
                    </div>
